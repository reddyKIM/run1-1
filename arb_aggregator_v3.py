...
def job_06(http: HttpClient, outdir: str, symbol: str) -> None:
    """Collect a single order book snapshot and compute depth bands."""
    ...
    # Try different depth limits to get a response quickly
    ...
    def band(levels: List[List[str]], is_bid: bool, pct: float) -> Tuple[float, float]:
        low = mid * (1 - pct) if is_bid else mid
        high = mid if is_bid else mid * (1 + pct)
        qty = 0.0
        usd = 0.0
        for p_str, q_str in levels:
            p = float(p_str)
            q = float(q_str)
            if is_bid and low <= p <= mid:
                qty += q
                usd += q * p
            elif (not is_bid) and mid <= p <= high:
                qty += q
                usd += q * p
        return qty, usd
...
def main() -> None:
    ...
    def run_once() -> None:
        ...
        ok, err = run_with_watchdog(
            lambda: job_06(http, args.out, args.symbol), args.timeout_06, "06"
        )
        if not ok:
            print(err)
        ok, err = run_with_watchdog(
